// Installs (or removes) the skill and its lifecycle hooks for Claude Code, Gemini CLI and Codex.
//   global:  hooks in ~/.claude/settings.json and ~/.gemini/settings.json; skill linked into
//            ~/.claude/skills and ~/.agents/skills (read by both Gemini CLI and Codex).
//   project: the same skill folder copied into <project>/.agents/skills, linked from .claude/skills,
//            hooks in the project's .claude/ and .gemini/ settings.
import fs from 'node:fs';
import os from 'node:os';
import path from 'node:path';
import { SKILL_ROOT, UserError, fail, parseArgs, values } from './lib.mjs';
...
function hasGlobalHooks(home) {
  ...
  return JSON.stringify(settings.hooks || {}).includes(NAME);
}

function main() {
  const args = parseArgs(process.argv.slice(2));
  const scope = values(args, 'scope')[0] || 'global';
  if (!['global', 'project'].includes(scope)) throw new UserError('--scope must be "global" or "project".', 'Global is recommended: --scope global');
  ...
  if (scope === 'global') {
    ...
    if (agents.includes('claude')) {
      ...
      if (!uninstall && hasGlobalHooks(home)) report.push('warning: global workspace-memory hooks are also installed; hooks will run twice (harmless, but slower). Prefer one scope.');
    }
    if (agents.includes('gemini')) {
  ...
  if (!uninstall && !dryRun) report.push('next: initialize a workspace with init.mjs --target <dir>, then start a new agent session there.');
